...
- **Messaging channels** — talk to your models outside the app; each conversation becomes a chat
  in the sidebar:
  - **WhatsApp**: unofficial via QR Code (embedded whatsmeow, no extra service) or Meta's **official Cloud API**.
...
- **Voice (TTS/STT)** built in (local Kokoro, no extra service; the model downloads on first
  use) or via any OpenAI-compatible endpoint, including **voice cloning**. Per-model voice (with blending). Voice input on the channels (audio → text).
- **Sound effects in the narration**: the AI drops `[[som: …]]` right where a sound happens —
  a door slamming, a sword clash, a monster snarling — and it shows up as an inline play button
  (or plays by itself, if you turn that on). Sounds are generated by ElevenLabs only when played
  and cached per description, so a repeated sound is instant and free. On in Imaginai whenever
  ElevenLabs is connected; for other models, via the "Sound effects" capability.
