...
| **external media generation × end of turn** | a Civitai workflow can outlive the tool's inline wait; the tool returned "still running" and the turn ended with the model PROMISING an image nobody would fetch. Waking a new turn would fix delivery but cost a full model call (whole chat context) just to hand over ready media — and the completion note would read as the USER speaking | `watch_workflow` queues the workflow; `_watch_poller` downloads the media and writes it as a **tool result on an empty-content assistant message** — the UI shows the card, the model context skips it (history requires `content`). Rule: **finished media needs delivery, not a model turn; a wake is for work that still needs deciding** |
...
| Pending media delivery | `civitai_service._pending_watch` | **yes** (`media_jobs`) | `recover_pending` re-queues unsettled workflows at boot (token re-read from secrets); paid media is never dropped silently |
