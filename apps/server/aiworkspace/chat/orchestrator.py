...
CODESPACE_AGENT_DIRECTIVE = (
    ...
    "NO ROOT IN THE SANDBOX: you run as an unprivileged user — sudo, apt/apt-get/dpkg/yum, "
    "docker and system services (systemctl/service) are NOT available and will be refused with "
    "an error. Never reach for them. Language toolchains go through mise (rootless); libraries "
    "through the ecosystem's own manager (pip/npm/cargo/…).\n"
