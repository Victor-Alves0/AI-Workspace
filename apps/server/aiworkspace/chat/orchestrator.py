...
CODESPACE_AGENT_DIRECTIVE = (
    ...
    "CONVERGE, DON'T RE-TREAD: define 'done' up front in the ledger plan, then drive the plan "
    "forward each turn — work through as many steps as you can before yielding. When the plan is "
    "complete OR the current approach is genuinely exhausted, STOP and give the final summary; do "
    "NOT emit another near-identical pass over the same ground. If you're stuck, change the "
    "approach (a different vector/tool) or say what you need — repeating the same conclusion is "
    "not progress.\n"
    "GROUND EVERY CONCLUSION: assert only what THIS session's tool output supports. Dynamic "
    "evidence beats static reasoning — if a live call returns 400/blocked/empty, that OVERRIDES a "
    "hypothesis you formed from reading code; record the reconciliation in the ledger. Never "
    "present intention, memory, or a plan as a result.\n"
    "CONSULT REFERENCES FIRST: before improvising, check what's already given — an attached skill "
    "or knowledge base often carries the domain's checklist / known-issue canon, and the code "
    "graph/files show how THIS project actually works. Use them so you don't reinvent a weaker "
    "version or miss the obvious known approach.\n"
...
def _delegate_tool(agents: list[dict[str, Any]], adhoc: bool = False,
    ...
    if isolation:
        props["isolated"] = {
            "type": "boolean",
            "description": "agent=new, in a Codespace project: work on its own git worktree "
...
def _delegate_target(args: dict, by_key: dict[str, dict[str, Any]], adhoc: bool,
                     isolation: bool) -> tuple[str, str, dict | None, str | None]:
    """(key, nome p/ exibir, spec do agente novo | None, erro | None)."""
    key = str(args.get("agent") or "")
    if key == "new" and adhoc:
    ...
    return key, spec.get("name", key), None, None
...
def _assemble_tools_and_prompt(
    *,
    ...
    # Subagentes: com a permissão ligada, o modelo (orquestrador) ganha a tool
    # `delegate` p/ acionar os agentes do usuário e/ou criar os seus (agent="new").
...
class _ToolDispatcher:
    ...
    async def _delegate(self, args: dict, tc: dict) -> AsyncGenerator[dict[str, Any], None]:
        if not self.subagents_on:
            ...
            return
        tcid = tc.get("id")
        if tcid in self.delegate_pre:  # já rodou em paralelo
            self.result = self.delegate_pre[tcid]
            yield {"type": "subagent", "status": "done", "id": tcid, "agent": (self.result.get("agent") if isinstance(self.result, dict) else None) or str(args.get("name") or args.get("agent") or "")}
            return
