...
def sign_shared_image_url(image_id: str, public_id: str, ttl_days: int = 3650) -> str:
    """URL de mídia para UMA conversa publicamente compartilhada.

    Diferente da URL privada, o token inclui o ``public_id`` atual do chat. Assim,
    revogar ou rotacionar o compartilhamento invalida imediatamente todas as URLs
    emitidas para ele, mesmo antes do vencimento criptográfico.
    """
    now = int(time.time())
    tok = jwt.encode(
        {"img": image_id, "share": public_id, "iat": now, "exp": now + ttl_days * 86400},
        get_settings().app_secret,
        algorithm="HS256",
    )
    return f"/images/{image_id}?s={tok}"


def verify_shared_image_token(image_id: str, token: str) -> str | None:
    try:
        data = jwt.decode(token, get_settings().app_secret, algorithms=["HS256"])
    except jwt.PyJWTError:
        return None
    share = data.get("share")
    return str(share) if data.get("img") == image_id and isinstance(share, str) and share else None
