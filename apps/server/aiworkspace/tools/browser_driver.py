...
class BrowserDriver:
    ...
    def _run_loop(self) -> None:
        ...
        self._loop.create_task(self._reaper())
    ...
    async def _ensure_browser(self) -> None:
        ...
        s = get_settings()
        base = (s.browser_ws_url or "").strip()
        if not base:
            raise RuntimeError("browser service not configured (set BROWSER_WS_URL)")
        token = (s.browser_token or "").strip()
        self._endpoint = f"{base}?token={token}" if token else base
        if self._pw is None:
            self._pw = await async_playwright().start()
        self._browser = await self._pw.chromium.connect_over_cdp(self._endpoint)

    async def _get_session(self, key: str) -> _Session:
        ...
        ctx = await self._browser.new_context(user_agent=_UA, viewport={"width": 1280, "height": 900})
        ctx.set_default_timeout(_ACTION_TIMEOUT_MS)
        page = await ctx.new_page()
        sess = _Session(key, ctx, page)
    ...
    # ------------------------------ ações ----------------------------------- #
    async def _act_goto(self, key: str, url: str) -> dict[str, Any]:
        sess = await self._get_session(key)
        await sess.page.goto(url, wait_until="domcontentloaded", timeout=_NAV_TIMEOUT_MS)
        sess.touch()
        return await self._state(sess.page)

    async def _act_read(self, key: str) -> dict[str, Any]:
        sess = await self._require(key)
        sess.touch()
        return await self._state(sess.page)

    async def _act_click(self, key: str, target: str) -> dict[str, Any]:
        sess = await self._require(key)
    ...
    async def _act_type(self, key: str, target: str, text: str, submit: bool) -> dict[str, Any]:
        sess = await self._require(key)
    ...
    async def _act_scroll(self, key: str, direction: str) -> dict[str, Any]:
        sess = await self._require(key)
    ...
    # --------------------------- API síncrona ------------------------------- #
    def goto(self, key: str, url: str) -> dict[str, Any]:
        return self._submit(self._act_goto(key, url), timeout=_NAV_TIMEOUT_MS / 1000 + 10)

    def read(self, key: str) -> dict[str, Any]:
        return self._submit(self._act_read(key), timeout=_ACTION_TIMEOUT_MS / 1000 + 10)

    def click(self, key: str, target: str) -> dict[str, Any]:
        return self._submit(self._act_click(key, target), timeout=_NAV_TIMEOUT_MS / 1000 + 10)

    def type(self, key: str, target: str, text: str, submit: bool) -> dict[str, Any]:
        return self._submit(self._act_type(key, target, text, submit), timeout=_NAV_TIMEOUT_MS / 1000 + 10)

    def scroll(self, key: str, direction: str) -> dict[str, Any]:
        return self._submit(self._act_scroll(key, direction), timeout=_ACTION_TIMEOUT_MS / 1000 + 10)

    def back(self, key: str) -> dict[str, Any]:
        return self._submit(self._act_back(key), timeout=_NAV_TIMEOUT_MS / 1000 + 10)

    def screenshot(self, key: str) -> bytes:
        return self._submit(self._act_screenshot(key), timeout=_ACTION_TIMEOUT_MS / 1000 + 10)

    def close(self, key: str) -> dict[str, Any]:
        self._submit(self._close_session(key), timeout=15)
