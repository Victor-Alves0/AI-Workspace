...
async def _run_one(connection_id: uuid.UUID, m: dict[str, Any]) -> None:
    ...
    async with SessionLocal() as db:
        ...
        # Nota de voz/áudio → baixa via getFile e deixa o Audio Router transcrever
        attachments: list[dict[str, Any]] = []
        audio_router = None
        if m.get("audio_file_id"):
            audio_router = await _audio_router_config(db, user, mc)
            try:
                raw = await telegram_api.get_file_bytes(token, m["audio_file_id"])
                b64 = base64.b64encode(raw).decode()
                mime = m.get("audio_mime") or "audio/ogg"
                attachments.append({
        ...
        extra_parts = [
            ...
            f"avoid markdown headings/tables. Match the contact's language."
        ]
