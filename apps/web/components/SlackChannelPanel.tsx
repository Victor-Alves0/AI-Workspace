...
interface SlackChannelConn {
  ...
  enabled: boolean;
  state: { status?: string; last_error?: string | null; last_event_at?: string };
  threads: number;
}
...
/** Tela "Canal do Slack" (Socket Mode): conecta um app do Slack (Bot Token + App-Level
 *  Token), associa a um modelo e configura filtros/memória. As mensagens do Slack viram
 *  Chats e a IA responde no Slack. Distinto da FERRAMENTA slack.workspace.manage. */
export default function SlackChannelPanel({ onBack }: { onBack: () => void }) {
  const confirm = useConfirm();
  const [conns, setConns] = useState<SlackChannelConn[] | null>(null);
  ...
  const load = useCallback(async () => {
    try { setConns(await api.get<SlackChannelConn[]>("/integrations/slack-channel/connections")); }
    catch { setConns([]); }
  }, []);
  useEffect(() => {
    load();
  ...
  async function connect() {
    ...
    try {
      ...
      await api.post("/integrations/slack-channel/connections", {
        bot_token: botToken.trim(), app_token: appToken.trim(), label: newLabel.trim(),
  ...
  async function patch(id: string, body: Partial<SlackChannelConn> & { model_config_id?: string | null }) {
    await api.patch(`/integrations/slack-channel/connections/${id}`, body).catch(() => {});
    load();
  }
