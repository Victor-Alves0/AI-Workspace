...
export function ImaginaiWorldBuilder({ campaignId, onClose }: { campaignId: string; onClose: () => void }) {
  ...
  // golpe do inimigo no turno dele (vazio = ataque padrão do servidor, +3 / 1d6+1)
  ...
  async function create(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (kind === "npc" || kind === "creature") {
        ...
        const dnd: Record<string, unknown> = { hp: { current: hp, max: hp }, armor_class: ac, conditions: [] };
        const damage = attackDamage.trim().toLowerCase().replace(/\s+/g, "");
        if (/^(\d+d\d+|\d+)([+-]\d+)?$/.test(damage)) {
          dnd.attacks = [{
            key: "attack", name: "Ataque", damage,
            attack_modifier: Math.max(-5, Math.min(30, Number.parseInt(attackBonus, 10) || 0)),
          }];
        }
        state.dnd5e = dnd;
